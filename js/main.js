...
if (currentPath === "/" || currentPath.endsWith("index.html")) {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (activeId !== id) {
          ...
          const targetLink = navLinks.filter(`[href="#${id}"]`);
    ...
    root: null,
    threshold: 0.55,     // 👈 تشخیص دقیق‌تر (نصف سکشن در دید باشه)
    rootMargin: "0px 0px -20% 0px" // 👈 کمک می‌کنه خیلی بهتر تشخیص بده
  });
  ...
  navLinks.on("click", function (e) {
    ...
    if (href.startsWith("#")) {
      ...
      $("html, body").animate({
        scrollTop: $(href).offset().top - 20 // 👈 بهتر می‌شینه دقیق
      }, 500);
    }
  });
